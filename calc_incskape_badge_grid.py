...
class CalcInkscapeBadgeGrid():
    """Calculate and Create Inkscape TemplateFile."""
    ...
    def calc_xy_array_pos(self):
        """Calculate x and y arrays for aux- & cutting-positions."""
    ...
    def get_auxiliary_line_horrizontal(self, pos_x, pos_y, guide) -> str:
        """Create XML String for horr. auxiliary lines."""
        auxiliary_line_horrizontal = (
            f"    <sodipodi:guide\n"
    ...
    def get_auxiliary_line_vertical(self, pos_x, pos_y, guide) -> str:
        """Create XML String for one vert. auxiliary line."""
        auxiliary_line_vertical = (
            f"    <sodipodi:guide\n"
            f'       position="{pos_x},{pos_y}"\n'
            f'       orientation="1,0"\n'
            f'       id="guide{guide}" />\n'
    ...
    def get_cutting_line_horrizontal(self, pos_y, abst_x,
                                     first_vert, last_vert, path_id) -> str:
        """Create XML String for one horr. cutting line pairs."""
        # print(first_vert, last_vert)
        # print(last_vert - abst_x)
        # print(paper_width - abst_x)
        cutting_line_horrizontal = (
            f"    <path\n"
            f'       style="fill:none;stroke:#000000;stroke-width:0.264583px;'
            f'stroke-linecap:butt;stroke-linejoin:miter;stroke-opacity:1"\n'
            f'       d="M {abst_x},{pos_y} H {first_vert - abst_x}"\n'
            f'       id="path{path_id}" />\n'
        )
        cutting_line_horrizontal = cutting_line_horrizontal + (
            f"    <path\n"
            f'       style="fill:none;stroke:#000000;stroke-width:0.264583px;'
            f'stroke-linecap:butt;stroke-linejoin:miter;stroke-opacity:1"\n'
            f'       d="M {last_vert  + abst_x},{pos_y} '
            f'H {self.paper_width -  abst_x}"\n'
            f'       id="path{path_id + 1}" />\n'
        )
        return cutting_line_horrizontal

    def get_cutting_line_vertical(self, pos_x, abst_y,
                                  first_horr, last_horr, path_id) -> str:
        """Create XML String for one vert. cutting line pairs."""
        # print(first_horr, last_horr)
        # print(last_horr - abst_y)
        # print(paper_height + abst_y)
        cutting_line_vertical = (
            f"    <path\n"
            f'       style="fill:none;stroke:#000000;stroke-width:0.264583px;'
            f'stroke-linecap:butt;stroke-linejoin:miter;stroke-opacity:1"\n'
        ...
        cutting_line_vertical = cutting_line_vertical + (
            f"    <path\n"
            f'       style="fill:none;stroke:#000000;stroke-width:0.264583px;'
            f'stroke-linecap:butt;stroke-linejoin:miter;stroke-opacity:1"\n'
            f'       d="M  {pos_x},{last_horr + abst_y } '
            f'V {self.paper_height - abst_y}"\n'
            f'       id="path{path_id+1}" />\n'
        )
        return cutting_line_vertical

    def create_auxiliary_lines(self) -> str:
        """Create XML String for all auxiliary lines."""
        auxiliary_lines = ""
        guide = 10
        guide_count = 0
        for pos_x in self.pos_x_array:
            auxiliary_lines = (auxiliary_lines +
                               self.get_auxiliary_line_vertical(
                                    pos_x, 0.0, guide))
            guide += 10
            guide_count += 1
        ...
        for pos_y in self.pos_y_array:
            auxiliary_lines = (auxiliary_lines +
                               self.get_auxiliary_line_horrizontal(
                                    0.0, pos_y, guide))
            guide += 10
            guide_count += 1
    ...
    def create_cutting_lines(self) -> str:
        """Create XML String for all cutting lines."""
        cutting_lines = ""
        path_count = 0
        path = 10
        abst_y = 2.0
        abst_x = 2.0
        for pos_x in self.pos_x_array:
            cutting_lines = cutting_lines + self.get_cutting_line_vertical(
                pos_x, abst_y, self.pos_y_array[0], self.pos_y_array[-1], path
            )
            path += 10
            path_count += 1

        for pos_y in self.pos_y_array:
            cutting_lines = cutting_lines + self.get_cutting_line_horrizontal(
                pos_y, abst_x, self.pos_x_array[0], self.pos_x_array[-1], path
            )
            path += 10
            path_count += 1
        # print(cutting_lines)
    ...
    def get_start_end_auxiliary_line_index(self, data):
        """Find start and end position in XML data to insert aux lines."""
    ...
    def get_start_end_cutting_line_index(self, data):
        """Find start and end position in XML data to insert cutting lines."""
        start_line = "<g"
        start_index = None
        end_index = None
        for i, line in enumerate(data):
    ...
    def write_svg_data(self, file_name=None):
        """Write SVG data to file, when finished."""
    ...
    def main(self):
        """Prepair full xml Inkscape string.

        SVG data have to be stored seperate.
        """
        ...
        self.offset_width = (self.paper_width -
                             self.count_with * self.badge_width) / 2
        # self.offset_width

        self.offset_heigth = (
            self.paper_height - self.count_height * self.bagde_height
        ) / 2
        # self.offset_heigth
        ...
        start_index, end_index = self.get_start_end_auxiliary_line_index(
                                    self.data)
        ...
        start_index, end_index = self.get_start_end_cutting_line_index(
                                    self.data)
        ...
        self.new_tamplate_file_name = self.tamplate_file_name.replace(
            ".svg", f"_({self.badge_width}-{self.bagde_height}).svg"
        )
