...
class badges_helper():
    ...
    def read_pdf_files_in_folder(self, pdf_folder = None):
        '''Function return all PDFs-files in folder "pdf_folder" return as array.
    ...
    def main(self, df, svg_template=None, enable_clear_template = True):
        ...
        for index, row in df.iterrows():
            ...
            if (not self.PATTERN_NACHNAME in current) and (not self.PATTERN_VORNAME in current):
        ...
        if enable_clear_template:
            ...
            while clear_template:
                if (self.PATTERN_NACHNAME in current) and (self.PATTERN_VORNAME in current):
                    firstname = " "
                    lastname = " "
                    orga = " "
                    current = current.replace(self.PATTERN_VORNAME, firstname, 1)
